...
def _find_matugen_json() -> str | None:
    ...
    optional ``matugen_colors_file`` key in tidal-tui's ``config.json``.
    """
    candidates: list[str] = []
    ...
    # user-specified path from tidal-tui config
    try:
        import tidaltui.tidal_client as tc
...
def _dark_mode() -> bool:
    return os.environ.get("TIDAL_TUI_MATUGEN_LIGHT", "").lower() not in (
        "1", "true", "yes",
    )
...
def load_theme_from_file(path: str, *, dark: bool | None = None) -> Theme | None:
    ...
    Args:
        ...
        dark: If ``None`` (default), the ``TIDAL_TUI_MATUGEN_LIGHT`` env var
