...
def _cmd_import_spotify(path: str) -> None:
    from tidaltui.play_count_store import PlayCountStore
    ...
    print(
        ...
        "Run tidal-tui and press s to cycle to favourites (★) or discovery (⊕)."
    )
...
def main() -> None:
    ...
    sp = sub.add_parser(
        ...
        description=(
            "Import your Spotify listening history into tidal-tui's play count store. "
    ...
    from tidaltui.app import TidalTUIApp
    from tidaltui.tidal_client import TidalClient

    client = TidalClient()
    client.ensure_login()
    TidalTUIApp(client=client).run()
