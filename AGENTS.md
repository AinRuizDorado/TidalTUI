...
- **Help popup**: `?` (`question_mark`) opens a centered `HelpModal` (`Screen` subclass, dimmed `$background 75%`, rounded border) listing hotkeys — generated from `TidalTUIApp.BINDINGS` + `_LOCAL_KEYS` as a styled `Text` (never markup strings: keys like `[` would break Rich markup). The modal blocks every key except `escape`/`?` (close) and `j`/`k` (scroll the body) via a `_check_bindings` override — without it, App bindings (space/n/p...) fire behind the modal. The App's `on_key` also early-returns while the modal is up. A persistent `#help-hint` label ("Press ? for Help") sits top-left.
